...
import { updateRadar } from '../services/radar';
import type { Radar } from '../types/database';
import { Button } from './Button';

interface RenameRadarModalProps {
  radar: Radar;
  onClose: () => void;
  onRenamed?: (radar: Radar) => void;
}

const MAX_NAME_LENGTH = 50;

export function RenameRadarModal({ radar, onClose, onRenamed }: RenameRadarModalProps) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (!trimmedName || trimmedName === radar.name) {
      return;
    }
    ...
    try {
      const updatedRadar = await updateRadar(radar.id, trimmedName);

      // Invalidate caches to refresh sidebar and current radar
      void queryClient.invalidateQueries({ queryKey: ['radars'] });
      void queryClient.invalidateQueries({ queryKey: ['radar', radar.id] });

      onRenamed?.(updatedRadar);
      onClose();
    } catch (err) {
      const message = err instanceof Error ? err.message : 'Failed to rename radar';
      setError(message);
  ...
  return (
    <Dialog open={true} onClose={handleClose} className="relative z-50">
      ...
      <div className="fixed inset-0 flex items-center justify-center p-4">
        ...
        >
          <DialogTitle className="text-lg font-semibold text-gray-900">Rename Radar</DialogTitle>
